    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for index, apartment in enumerate(apartments):
                ...
                get_time_post = apartment.find('p', class_='css-1mwdrlh').get_text(strip=True).split('-') if apartment.find('p', class_='css-1mwdrlh') else None

 
                if get_time_post is None or article_id in data["OLX"]:
                    continue
                
                if 'Сьогодні' in get_time_post[-1] or str(local_time.day - 1) in get_time_post[-1]:
                    

                    if "Голосіївський" not in get_time_post[0]:
                        continue
